...
if(isset($_POST['btnsrc']))
		{
$dt=$_POST["src"];
$sql = "select * from record where  v_number like '$dt%' order by r_id desc ";
                $result = $con->query($sql);
...
$sql= "select * from record where  v_number like '$dt%' order by r_id desc LIMIT $record_index, $limit";
...
$sql = "SELECT COUNT(*) FROM record where  v_number like '$dt%' order by r_id desc"; 
...
$total_pages = ceil($total_records / $limit);  
   ...
   echo "<ul>"; 
   echo "<li><a href='pn_search.php?page=".($page-1)."'>Previous</a></li>"; 
for ($i=1; $i<=$total_pages; $i++) {  

 

  
    echo "<li><a style='width:10%;' href='pn_search.php?page=".$i."'>".$i."</a></li>";
    

                
};  echo "<li><a href='pn_search.php?page=".($page+1)."'>NEXT</a></li>";
   echo"</ul></div>";
		}
    mysqli_close($con);
			}
