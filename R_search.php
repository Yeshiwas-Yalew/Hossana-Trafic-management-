...
if(isset($_POST['btnsrc']))
		{
$dt=$_POST["reason"];
$sql = "select * from record where  reason like '$dt%' order by r_id desc ";
                $result = $con->query($sql);
...
$sql= "select * from record where  reason like '$dt%' order by r_id desc LIMIT $record_index, $limit";
...
$sql = "SELECT COUNT(*) FROM record where  reason like '$dt%' order by r_id desc"; 
...
$total_pages = ceil($total_records / $limit);  
   ...
   echo "<ul>"; 
   echo "<li><a href='R_search.php?page=".($page-1)."'>Previous</a></li>"; 
for ($i=1; $i<=$total_pages; $i++) {  

 

  
    echo "<li><a style='width:10%;' href='R_search.php?page=".$i."'>".$i."</a></li>";
    

                
};  echo "<li><a href='R_search.php?page=".($page+1)."'>NEXT</a></li>";
   echo"</ul></div>";
		}
    mysqli_close($con);
			}
